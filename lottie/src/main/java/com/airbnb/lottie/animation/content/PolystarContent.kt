...
class PolystarContent(
    ...
    override val name: String = polystarShape.name
    private val type = polystarShape.type
    private val hidden = polystarShape.isHidden
    private val isReversed = polystarShape.isReversed
    private val pointsAnimation: BaseKeyframeAnimation<Float, Float> = polystarShape.points.createAnimation()
    private val positionAnimation: BaseKeyframeAnimation<PointF, PointF>?
    private val rotationAnimation: BaseKeyframeAnimation<Float, Float>?
    ...
    private var innerRoundednessAnimation: BaseKeyframeAnimation<Float, Float>? = null
    private val outerRoundednessAnimation: BaseKeyframeAnimation<Float, Float>?

    private val trimPaths = CompoundTrimPathContent()
    private var isPathValid = false

    init {
        positionAnimation = polystarShape.position.createAnimation()
        rotationAnimation = polystarShape.rotation.createAnimation()
        outerRadiusAnimation = polystarShape.outerRadius.createAnimation()
        outerRoundednessAnimation = polystarShape.outerRoundedness.createAnimation()
        if (type == PolystarShape.Type.STAR) {
            innerRadiusAnimation = polystarShape.innerRadius.createAnimation()
            innerRoundednessAnimation = polystarShape.innerRoundedness.createAnimation()
        } else {
    ...
    private fun createStarPath() {
        val points = pointsAnimation.value
        var currentAngle = (if (rotationAnimation == null) 0f else rotationAnimation.value)!!.toDouble()
        ...
        val outerRadius = outerRadiusAnimation.value
        val innerRadius = innerRadiusAnimation!!.value
        ...
        while (i < numPoints) {
            ...
            if (innerRoundedness == 0f && outerRoundedness == 0f) {
                ...
                val cp1Theta = (atan2(previousY.toDouble(), previousX.toDouble()) - Math.PI / 2f).toFloat()
    ...
    private fun createPolygonPath() {
        ...
        val roundedness = outerRoundednessAnimation!!.value!! / 100f
        val radius = outerRadiusAnimation.value
        var x: Float
        var y: Float
        var previousX: Float
        var previousY: Float
        ...
        while (i < numPoints) {
            ...
            if (roundedness != 0f) {
                ...
                if (i.toDouble() == numPoints - 1) {
                    ...
                    lastSegmentPathMeasure.getPosTan(lastSegmentPathMeasure.length * 0.9999f, lastSegmentPosition, null)
