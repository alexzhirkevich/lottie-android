...
abstract class BaseKeyframeAnimation<K : Any, A : Any?> internal constructor(keyframes: List<Keyframe<K>>) {
    ...
    open fun setProgress(@FloatRange(from = 0.0, to = 1.0) progress: Float) {
        ...
        if (isTraceEnabled()) {
            beginSection("BaseKeyframeAnimation#setProgress")
        }
        if (keyframesWrapper.isEmpty) {
            if (isTraceEnabled()) {
                endSection("BaseKeyframeAnimation#setProgress")
            }
            return
        }
    ...
    open fun notifyListeners() {
        if (isTraceEnabled()) {
            beginSection("BaseKeyframeAnimation#notifyListeners")
        }
        for (i in listeners.indices) {
            listeners[i].onValueChanged()
        }
        if (isTraceEnabled()) {
            endSection("BaseKeyframeAnimation#notifyListeners")
        }
    }
    ...
    val currentKeyframe: Keyframe<K>
        get() {
            if (isTraceEnabled()) {
                beginSection("BaseKeyframeAnimation#getCurrentKeyframe")
            }
            val keyframe: Keyframe<K> = keyframesWrapper.currentKeyframe
            if (isTraceEnabled()) {
                endSection("BaseKeyframeAnimation#getCurrentKeyframe")
            }
            return keyframe
        }
    ...
    val linearCurrentKeyframeProgress: Float
        ...
        get() {
            ...
            val progressIntoFrame = _progress - keyframe.startProgress
    ...
    open val value: A
        get() {
            ...
            if (cachedGetValue != null && valueCallback == null && keyframesWrapper.isCachedValueEnabled(linearProgress)) {
     ...
     */
    protected open fun getValue(keyframe: Keyframe<K>, linearKeyframeProgress: Float, xKeyframeProgress: Float, yKeyframeProgress: Float): A {
