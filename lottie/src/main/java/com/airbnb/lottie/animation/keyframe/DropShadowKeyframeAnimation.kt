...
class DropShadowKeyframeAnimation(
    ...
    private val color: BaseKeyframeAnimation<Int, Int> = dropShadowEffect.color.createAnimation()
    ...
    fun applyTo(paint: Paint) {
        ...
        val directionRad = (direction.value!!.toDouble()) * DEG_TO_RAD
        val distance = distance.value
        val x = (sin(directionRad).toFloat()) * distance
        val y = (cos(directionRad + Math.PI).toFloat()) * distance
        val baseColor = color.value
        val opacity = Math.round(opacity.value!!)
        val color = Color.argb(opacity, Color.red(baseColor), Color.green(baseColor), Color.blue(baseColor))
    ...
    fun setOpacityCallback(callback: LottieValueCallback<Float>?) {
        ...
        opacity.setValueCallback(object : LottieValueCallback<Float>() {
            ...
            override fun getValue(frameInfo: LottieFrameInfo<Float>): Float {
                val value = callback.getValue(frameInfo) ?: return 255f
