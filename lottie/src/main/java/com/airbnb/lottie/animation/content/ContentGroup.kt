...
class ContentGroup internal constructor(
  private val lottieDrawable: LottieDrawable, layer: BaseLayer?,
  override val name: String,
  private val hidden: Boolean,
  @JvmField val contents: MutableList<Content>,
  transform: AnimatableTransform?
    ...
    private val offScreenRectF = RectF()
    ...
    constructor(
        ...
        shapeGroup.name,
        shapeGroup.isHidden,
        contentsFromModels(lottieDrawable, composition, layer, shapeGroup.items).toMutableList(),
        findTransform(shapeGroup.items)
    )

    init {
        if (transform != null) {
            transformAnimation = transform.createAnimation()?.also {
                it.addAnimationsToLayer(layer!!)
                it.addListener(this)
            }
        }
    ...
    override fun getPath(): Path {
        ...
        for (i in contents.indices.reversed()) {
            val content = contents[i]
            if (content is PathContent) {
                path.addPath(content.getPath(), matrix)
            }
        }
        return path
    }
    ...
    override fun draw(canvas: Canvas, parentMatrix: Matrix, parentAlpha: Int) {
        ...
        val layerAlpha: Int = transformAnimation?.let {
            matrix.preConcat(it.getMatrix())
            val opacity = if (it.getOpacity() == null) 100 else it.getOpacity()!!.value
            ((opacity / 100f * parentAlpha / 255f) * 255).toInt()
        } ?: parentAlpha

        // Apply off-screen rendering only when needed in order to improve rendering performance.
        val isRenderingWithOffScreen = lottieDrawable.isApplyingOpacityToLayersEnabled && hasTwoOrMoreDrawableContent() && layerAlpha != 255
        if (isRenderingWithOffScreen) {
            offScreenRectF[0f, 0f, 0f] = 0f
            getBounds(offScreenRectF, matrix, true)
            offScreenPaint.alpha = layerAlpha
            saveLayerCompat(canvas, offScreenRectF, offScreenPaint)
        }
        ...
        if (keyPath.propagateToChildren(name, depth)) {
            ...
            for (i in contents.indices) {
                ...
                if (content is KeyPathElement) {
                    val element = content as KeyPathElement
