 ...
 */
open class LottieValueAnimator : BaseLottieAnimator(), Choreographer.FrameCallback {
    /**
    ...
    override fun doFrame(frameTimeNanos: Long) {
        ...
        if (isTraceEnabled()) {
            beginSection("LottieValueAnimator#doFrame")
        }
        ...
        verifyFrame()
        if (isTraceEnabled()) {
            endSection("LottieValueAnimator#doFrame")
        }
    }
    ...
    private val frameDurationNs: Float
        get() {
            ...
            return (Utils.SECOND_IN_NANOS / composition!!.frameRate / abs(speed.toDouble())).toFloat()
        }
    ...
    fun setComposition(composition: LottieComposition) {
        ...
        if (keepMinAndMaxFrames) {
            setMinAndMaxFrames(
                max(minFrame.toDouble(), composition.startFrame.toDouble()).toFloat(),
                min(maxFrame.toDouble(), composition.endFrame.toDouble()).toFloat()
            )
        } else {
            setMinAndMaxFrames(composition.startFrame.toInt().toFloat(), composition.endFrame.toInt().toFloat())
        }
    ...
    fun setMinFrame(minFrame: Float) {
    ...
    fun setMinAndMaxFrames(minFrame: Float, maxFrame: Float) {
        ...
        val compositionMinFrame = if (composition == null) -Float.MAX_VALUE else composition!!.startFrame
        val compositionMaxFrame = if (composition == null) Float.MAX_VALUE else composition!!.endFrame
    ...
    fun getMinFrame(): Float {
        ...
        return if (minFrame.toInt() == Int.MIN_VALUE)
            composition!!.startFrame else minFrame
    }
    ...
    fun getMaxFrame(): Float {
        ...
        return if (maxFrame.toInt() == Int.MAX_VALUE)
            composition!!.endFrame else maxFrame
    }
    ...
    protected open fun postFrameCallback() {
        if (isRunning) {
    ...
    private fun verifyFrame() {
        ...
        check(!(frame < minFrame || frame > maxFrame)) { String.format("Frame must be [%f,%f]. It is %f", minFrame, maxFrame, frame) }
    }
}
