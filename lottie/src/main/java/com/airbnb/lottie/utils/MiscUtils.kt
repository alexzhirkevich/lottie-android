...
object MiscUtils {
    ...
    fun getPathFromData(shapeData: ShapeData, outPath: Path) {
        ...
        for (i in shapeData.getCurves().indices) {
            val curveData = shapeData.getCurves()[i]
    ...
    @JvmStatic
    fun lerp(a: Float, b: Float, @FloatRange(from = 0.0, to = 1.0) percentage: Float): Float {
        return a + percentage * (b - a)
    }

    fun lerp(a: Double, b: Double, @FloatRange(from = 0.0, to = 1.0) percentage: Double): Double {
        return a + percentage * (b - a)
    }

    @JvmStatic
    fun lerp(a: Int, b: Int, @FloatRange(from = 0.0, to = 1.0) percentage: Float): Int {
